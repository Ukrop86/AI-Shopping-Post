...
const INSTAGRAM_FORMAT_RULES: Record<InstagramCaptionFormat, string> = {
  reels: `
Це підпис під Reels — коротке вертикальне відео товару на моделі.
- Перше речення має чіпляти: його видно ще до розгортання підпису.
- Дуже коротко, 2-4 речення. Люди дивляться відео, а не читають.
- Обов'язково назви ціну.`,
  slideshow: `
Це підпис під Reels зі слайдшоу з фото товару (кадри змінюються під музику).
- Перше речення має чіпляти.
- Коротко, 2-4 речення, без переліку характеристик — їх видно на кадрах.
- Обов'язково назви ціну.`,
  carousel: `
Це підпис під карусель фото в стрічці.
- Тут можна детальніше: посадка, тканина, наявні розміри.
- 4-7 речень, короткими абзацами.
- Обов'язково назви ціну і згадай, що більше фото — гортати далі.`,
};

export function instagramFormatPrompt(product: ProductInput, format: InstagramCaptionFormat) {
  return `
${commonRules(product)}

Створи готовий підпис для Instagram.
${INSTAGRAM_FORMAT_RULES[format]}

Правила Instagram:
- Без HTML і markdown.
- Теплий і живий тон, але без перебільшень.
- CTA без "пишіть в дірект", якщо це не вказано користувачем.
- 5-10 релевантних українських хештегів у кінці.
- Не використовуй англійські фрази.
